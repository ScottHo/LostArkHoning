        ...
        for protection in range(3):
            for blessing in range(7):
                for grace in range(13):
                    key = fullKey + \
                        "{}-{}-{}... ".format(grace, blessing, protection)
                    cost = self.getCost(grace, blessing, protection)
                    chance = self.getChance(
                        grace, blessing, protection, previousFails+1)
                    if artisanEnergy >= 1 or chance >= 1:
                        ...
                        artisanEnergyToGain = self.getArtisanEnergy(
                            grace, blessing, protection)
                        self.storeNextCost(previousFails + 1, artisanEnergy + artisanEnergyToGain,
                                           key, accumulatedChance + thisChance, totalPreviousCost + thisCost)
        return

    def findBestRate(self):
        """
            ...
            for blessing in range(7):
                for grace in range(13):
                    ...
                    chance = self.getChance(grace, blessing, protection, 0)\
                        artisanEnergy = self.getArtisanEnergy(
                            grace, blessing, protection)
                    self.storeNextCost(0, artisanEnergy, key, chance, cost)

        print(self.metadata.getString(), self.minKey,
              "Average Cost: {}".format(round(self.minCost, 2)))
        return


if __name__ == '__main__':
    Hone(Metadata7A()).findBestRate()
    Hone(Metadata8A()).findBestRate()
    Hone(Metadata9A()).findBestRate()
    Hone(Metadata10A()).findBestRate()
